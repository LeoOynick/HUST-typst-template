...
列表后的正文，应当有缩进。这里加入一个 \#indent() #highlight()[(在Typst 0.13后利用`set par(first-line-indent: (all: true)`，即可自动实现段落缩进)] 函数来手动生成段落缩进，是因为在目前的 typst 设计里，按英文排版的习惯，连续段落里的第一段不会缩进，也包括各种列表。
